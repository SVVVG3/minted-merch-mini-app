...
function PartnerDashboard() {
  ...
  const getStatusColor = (status) => {
    switch (status) {
      case 'assigned': return 'bg-orange-900/50 text-orange-400';
      case 'shipped': return 'bg-blue-900/50 text-blue-400';
      case 'payment_processing': return 'bg-yellow-900/50 text-yellow-400';
      case 'vendor_paid': return 'bg-teal-900/50 text-teal-400';
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        {orders.length > 0 && (
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-6">
            {/* Completed Payouts */}
            <div className="bg-teal-900/50 rounded-lg p-4 border border-teal-700/50">
              <div className="text-xs text-teal-400 font-medium uppercase tracking-wide">Completed Payouts</div>
            ...
            {/* Estimated Payouts */}
            <div className="bg-yellow-900/50 rounded-lg p-4 border border-yellow-700/50">
              <div className="text-xs text-yellow-400 font-medium uppercase tracking-wide">Est. Processing</div>
              <div className={`text-2xl font-bold mt-1 ${
            ...
            {/* Pending Orders */}
            <div className="bg-blue-900/50 rounded-lg p-4 border border-blue-700/50">
              <div className="text-xs text-blue-400 font-medium uppercase tracking-wide">Pending</div>
            ...
            <div className="bg-gray-800/50 rounded-lg p-4 border border-gray-700/50">
              <div className="text-xs text-gray-400 font-medium uppercase tracking-wide">Total Earnings</div>
              <div className={`text-2xl font-bold mt-1 ${
        ...
        <div className="bg-gray-800/50 rounded-lg shadow border border-gray-700/50">
          <div className="px-6 py-4 border-b border-gray-700 flex justify-between items-center">
            <h2 className="text-lg font-semibold text-white">Your Assigned Orders</h2>
            <button
              onClick={loadOrders}
          ...
          {loading ? (
            ...
            <div className="p-6 text-center">
              <div className="text-gray-400">No orders assigned to you yet.</div>
            </div>
          ) : (
            <>
              {/* Mobile Card View */}
                  ...
                  <div key={order.order_id} className="p-4 space-y-3">
                    <div className="flex justify-between items-start">
                      ...
                      <div className="text-right">
                        <div className="font-bold text-white">${order.amount_total}</div>
                        {/* Show estimated payout during payment processing */}
                        {order.status === 'payment_processing' && order.vendor_payout_estimated && (
                          <div className={`text-sm ${parseFloat(order.vendor_payout_estimated) < 0 ? 'text-red-400' : 'text-yellow-400'}`}>
                        ...
                        {order.status === 'vendor_paid' && order.vendor_payout_amount && (
                          <div className={`text-sm ${parseFloat(order.vendor_payout_amount) < 0 ? 'text-red-400' : 'text-teal-400'}`}>
                    ...
                    {partnerType === 'fulfillment' && (order.status === 'shipped' || order.status === 'payment_processing' || order.status === 'vendor_paid') && order.tracking_number && (
                      <div className="text-sm text-gray-400">
                        <div className="font-medium text-green-400">✅ Shipped</div>
                    ...
                    {partnerType === 'collab' && order.profiles && (
                      <div className="flex items-center space-x-2">
                        ...
                        <div>
                          <div className="font-medium text-white">@{order.profiles.username}</div>
              ...
              <div className="hidden sm:block overflow-x-auto">
                ...
                <tbody className="divide-y divide-gray-700">
                  {orders.map((order) => (
                    <tr key={order.order_id} className="hover:bg-gray-700/30">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-300">
                        {order.discount_code ? (
                          <div>
                            <div className="font-medium text-white">{order.discount_code}</div>
                            <div className="text-xs text-green-400">${parseFloat(order.discount_amount || 0).toFixed(2)}</div>
                          </div>
                        ) : (
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-sm">
                        ...
                        {order.status === 'vendor_paid' && order.vendor_payout_amount ? (
                          <div>
                            <div className={`font-medium ${parseFloat(order.vendor_payout_amount) < 0 ? 'text-red-400' : 'text-teal-400'}`}>
                          ...
                          <span className="text-teal-400 text-xs">Paid</span>
                        ) : order.status === 'payment_processing' && order.vendor_payout_estimated ? (
                          <div>
                            <div className={`font-medium ${parseFloat(order.vendor_payout_estimated) < 0 ? 'text-red-400' : 'text-yellow-400'}`}>
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-sm font-medium space-x-2">
                        <button
                          onClick={() => setSelectedOrder(order)}
                          className="text-[#3eb489] hover:text-[#4fd49f]"
                        >
                          {partnerType === 'fulfillment' ? 'View Details' : 'View Order'}
                        </button>
                      </td>
...
function OrderDetailModal({ order, partnerType, onClose, onUpdate, updating }) {
  ...
  return (
    <div className="fixed inset-0 bg-black/70 backdrop-blur-sm z-50 flex items-center justify-center p-4">
      <div className="bg-gradient-to-br from-gray-900 via-gray-800 to-gray-900 rounded-lg max-w-2xl w-full max-h-[90vh] overflow-y-auto border border-gray-700/50">
        <div className="p-6">
          <div className="flex justify-between items-center mb-6">
            <h2 className="text-xl font-bold text-white">Order {order.order_id}</h2>
            <button
              onClick={onClose}
          ...
          {partnerType === 'fulfillment' ? (
            ...
            order.profiles && (
              <div className="mb-6">
                <h3 className="text-lg font-semibold mb-3 text-white">Customer</h3>
                <div className="bg-gray-800/50 p-4 rounded-md border border-gray-700/50">
                  <div className="flex items-center space-x-3">
                    ...
                    <div className="text-sm text-gray-300">
                      <div className="font-medium text-lg text-white">@{order.profiles.username}</div>
                      {order.profiles.display_name && (
                        <div className="text-gray-400">{order.profiles.display_name}</div>
                      )}
          ...
          {/* Order Items */}
          <div className="mb-6">
            <h3 className="text-lg font-semibold mb-3 text-white">Order Items</h3>
            <div className="space-y-3">
              {order.order_items?.map((item, index) => {
                const productImage = item.product_data?.images?.[0]?.src || item.product_data?.image_url;
                return (
                  <div key={index} className="flex items-center space-x-3 bg-gray-800/50 p-3 rounded-md border border-gray-700/50">
                    {productImage && (
                      <img
                        src={productImage}
                    ...
                    <div className="flex-1">
                      <div className="text-sm font-medium text-white">{item.product_title}</div>
                      {item.variant_title && item.variant_title !== 'Default Title' && (
            ...
            {order.discount_code && (
              <div className="mt-4 bg-green-900/30 border border-green-700/50 rounded-md p-3">
                <div className="flex justify-between items-center">
                  <div>
                    <div className="text-sm font-medium text-white">Discount Applied</div>
                    <div className="text-xs text-gray-400">Code: {order.discount_code}</div>
                  </div>
            ...
            <div className="mt-4 pt-4 border-t border-gray-700">
              <div className="flex justify-between items-center">
                <div className="text-lg font-semibold text-white">Order Total</div>
                <div className="text-lg font-bold text-white">${parseFloat(order.amount_total).toFixed(2)}</div>
              </div>
            </div>

            {/* Estimated Payout - during payment processing */}
            {order.status === 'payment_processing' && order.vendor_payout_estimated && (
              <div className={`mt-4 ${parseFloat(order.vendor_payout_estimated) < 0 ? 'bg-red-900/30 border-red-700/50' : 'bg-yellow-900/30 border-yellow-700/50'} border rounded-md p-4`}>
                <div className="flex justify-between items-center">
                  <div>
                    <div className={`text-sm font-medium ${parseFloat(order.vendor_payout_estimated) < 0 ? 'text-red-400' : 'text-yellow-400'}`}>
                      ⏳ Estimated Payout
                    </div>
                    {order.payment_processing_at && (
                      <div className={`text-xs ${parseFloat(order.vendor_payout_estimated) < 0 ? 'text-red-400/70' : 'text-yellow-400/70'} mt-1`}>
                  ...
                  <div className={`text-xl font-bold ${parseFloat(order.vendor_payout_estimated) < 0 ? 'text-red-400' : 'text-yellow-300'}`}>
                    ${parseFloat(order.vendor_payout_estimated).toFixed(2)}
                  </div>
                </div>
                {order.vendor_payout_partner_notes && (
                  <div className={`text-xs ${parseFloat(order.vendor_payout_estimated) < 0 ? 'text-red-400/70' : 'text-yellow-400/70'} mt-2 pt-2 border-t ${parseFloat(order.vendor_payout_estimated) < 0 ? 'border-red-700/50' : 'border-yellow-700/50'}`}>
                    {order.vendor_payout_partner_notes}
                  </div>
                )}
              </div>
            ...
            {order.vendor_payout_amount && (
              <div className={`mt-4 ${parseFloat(order.vendor_payout_amount) < 0 ? 'bg-red-900/30 border-red-700/50' : 'bg-teal-900/30 border-teal-700/50'} border rounded-md p-4`}>
                <div className="flex justify-between items-center">
                  <div>
                    <div className={`text-sm font-medium ${parseFloat(order.vendor_payout_amount) < 0 ? 'text-red-400' : 'text-teal-400'}`}>
                      💰 Your Payout
                    </div>
                    {order.vendor_paid_at && (
                      <div className={`text-xs ${parseFloat(order.vendor_payout_amount) < 0 ? 'text-red-400/70' : 'text-teal-400/70'} mt-1`}>
                  ...
                  <div className={`text-xl font-bold ${parseFloat(order.vendor_payout_amount) < 0 ? 'text-red-400' : 'text-teal-300'}`}>
                    ${parseFloat(order.vendor_payout_amount).toFixed(2)}
                  </div>
                </div>
                {order.vendor_payout_partner_notes && (
                  <div className={`text-xs ${parseFloat(order.vendor_payout_amount) < 0 ? 'text-red-400/70' : 'text-teal-400/70'} mt-2 pt-2 border-t ${parseFloat(order.vendor_payout_amount) < 0 ? 'border-red-700/50' : 'border-teal-700/50'}`}>
                    {order.vendor_payout_partner_notes}
                  </div>
                )}
              </div>
          ...
          {partnerType === 'fulfillment' ? (
            order.status === 'shipped' || order.status === 'payment_processing' || order.status === 'vendor_paid' ? (
              <div className="space-y-4">
                {/* Shipped Status */}
                <div className="bg-green-900/30 border border-green-700/50 rounded-md p-3">
                  <div className="flex items-center">
                    <span className="text-green-400 mr-2">📦</span>
                    <div>
                      <p className="text-sm font-medium text-green-400">Order Shipped</p>
                      {order.shipped_at && (
                ...
                {(order.status === 'payment_processing' || order.status === 'vendor_paid') && order.payment_processing_at && (
                  <div className={`${order.vendor_payout_estimated && parseFloat(order.vendor_payout_estimated) < 0 ? 'bg-red-900/30 border-red-700/50' : 'bg-yellow-900/30 border-yellow-700/50'} border rounded-md p-3`}>
                    <div className="flex items-center justify-between">
                      <div className="flex items-center">
                        <span className={`${order.vendor_payout_estimated && parseFloat(order.vendor_payout_estimated) < 0 ? 'text-red-400' : 'text-yellow-400'} mr-2`}>⏳</span>
                        <div>
                          <p className={`text-sm font-medium ${order.vendor_payout_estimated && parseFloat(order.vendor_payout_estimated) < 0 ? 'text-red-400' : 'text-yellow-400'}`}>
                            {order.status === 'payment_processing' ? 'Vendor Payment Processing' : 'Payment Was Processing'}
                          </p>
                          <p className={`text-xs ${order.vendor_payout_estimated && parseFloat(order.vendor_payout_estimated) < 0 ? 'text-red-400/70' : 'text-yellow-400/70'}`}>
                      ...
                      {order.vendor_payout_estimated && (
                        <span className={`text-lg font-bold ${parseFloat(order.vendor_payout_estimated) < 0 ? 'text-red-400' : 'text-yellow-300'}`}>
                          ~${parseFloat(order.vendor_payout_estimated).toFixed(2)}
                        </span>
                      )}
                    </div>
                ...
                {/* Payout Status */}
                {order.status === 'vendor_paid' && (
                  <div className={`${order.vendor_payout_amount && parseFloat(order.vendor_payout_amount) < 0 ? 'bg-red-900/30 border-red-700/50' : 'bg-teal-900/30 border-teal-700/50'} border rounded-md p-3`}>
                    <div className="flex items-center justify-between">
                      <div className="flex items-center">
                        <span className={`${order.vendor_payout_amount && parseFloat(order.vendor_payout_amount) < 0 ? 'text-red-400' : 'text-teal-400'} mr-2`}>💰</span>
                        <div>
                          <p className={`text-sm font-medium ${order.vendor_payout_amount && parseFloat(order.vendor_payout_amount) < 0 ? 'text-red-400' : 'text-teal-400'}`}>Payout Processed</p>
                          {order.vendor_paid_at && (
                            <p className={`text-xs ${order.vendor_payout_amount && parseFloat(order.vendor_payout_amount) < 0 ? 'text-red-400/70' : 'text-teal-400/70'}`}>
                      ...
                      {order.vendor_payout_amount && (
                        <span className={`text-lg font-bold ${parseFloat(order.vendor_payout_amount) < 0 ? 'text-red-400' : 'text-teal-300'}`}>
                          ${parseFloat(order.vendor_payout_amount).toFixed(2)}
                        </span>
                      )}
                    </div>
                    {order.vendor_payout_partner_notes && (
                      <p className={`text-xs ${order.vendor_payout_amount && parseFloat(order.vendor_payout_amount) < 0 ? 'text-red-400/70' : 'text-teal-400/70'} mt-2 pt-2 border-t ${order.vendor_payout_amount && parseFloat(order.vendor_payout_amount) < 0 ? 'border-red-700/50' : 'border-teal-700/50'}`}>
                        {order.vendor_payout_partner_notes}
                      </p>
                    )}
                  </div>
                ...
                {order.tracking_number && (
                  <div className="bg-gray-800/50 rounded-md p-4 border border-gray-700/50">
                    <h4 className="font-medium text-white mb-2">Tracking Information</h4>
                    <div className="grid grid-cols-2 gap-4 text-sm">
                      <div>
                        <span className="text-gray-500">Tracking Number:</span>
                        <div className="font-medium text-white">{order.tracking_number}</div>
                      </div>
                      {order.carrier && (
                        <div>
                          <span className="text-gray-500">Carrier:</span>
                          <div className="font-medium text-white">{order.carrier}</div>
                        </div>
                      )}
                    </div>
                  </div>
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                <div className="bg-blue-900/30 border border-blue-700/50 rounded-md p-3 mb-4">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <input
                      ...
                      className="w-full px-3 py-2 bg-gray-800 border border-gray-700 rounded-md text-white placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-[#3eb489]"
                  ...
                  <div>
                    <label className="block text-sm font-medium text-gray-300 mb-2">
                      Carrier <span className="text-gray-500">(Optional)</span>
                    </label>
                    <input
                      type="text"
                      value={carrier}
                ...
                <div className="flex space-x-3 pt-4">
                  <button
                    type="button"
                    onClick={onClose}
                    className="flex-1 bg-gray-700 hover:bg-gray-600 text-white px-4 py-2 rounded-md disabled:opacity-50"
                    disabled={updating}
                  >
                    Cancel
                  </button>
