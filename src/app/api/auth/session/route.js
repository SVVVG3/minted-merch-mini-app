 ...
 * 
 * Uses official @farcaster/quick-auth library for cryptographic verification.
 * Falls back to structural validation if network errors occur (Vercel DNS issues).
 */
const quickAuthClient = createClient();

async function verifyFarcasterQuickAuth(token) {
  try {
    console.log('🔐 SECURE: Verifying Quick Auth JWT with official Farcaster library...');
    
    // Use official Farcaster Quick Auth library
    const payload = await quickAuthClient.verifyJwt({
      token,
      domain: process.env.NEXT_PUBLIC_URL || 'app.mintedmerch.shop',
    });
    
    console.log('✅ JWT signature cryptographically verified by Farcaster (official library)');
    ...
    // Check if this is InvalidTokenError from official library
